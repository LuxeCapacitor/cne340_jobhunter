...
# CNE 340
# JobHunter.py
# This is a program that scrapes for job openings and deposits them into a database on my WAMP server
...
def delete_job(cursor, jobdetails):
    job_id = jobdetails['id']
    query = "DELETE FROM jobs WHERE title = \"%s\"" % job_id
    return query_sql(cursor, query)
...
def add_or_delete_job(jobpage, cursor):
    ...
    for jobdetails in jobpage['jobs']:  # EXTRACTS EACH JOB FROM THE JOB LIST. It errored out until I specified jobs. This is because it needs to
        # look at the jobs dictionary from the API. https://careerkarma.com/blog/python-typeerror-int-object-is-not-iterable/
        ...
        if is_job_found:
            now = date.today()
            print("Found a new Job!: Wanna be a " + jobdetails["Title"] + " with " + jobdetails["company"] + '?')
